...
$(function(){
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            var popupPositionT = ($(window).height() - $('.main_popup_layer').height()) / 2;
            var popupPositionL = (($(window).width() - $('.main_popup_layer').width()) / 2 < 0) ? '15' : ($(window).width() - $('.main_popup_layer').width()) / 2;
            $('.main_popup_layer').css('top', popupPositionT+'px');
            $('.main_popup_layer').css('left', popupPositionL+'px');
        }
    });
...
function gd_member_event_fixed_popup(data)
{
    ...
    popupHtml +=
        '<div id="memberEventPopup_'+ data.eventType +'" class="main_popup_layer" style="position: absolute; /*overflow: hidden;*/ z-index: 300; width: 406px; height: 472px; padding-bottom: 10px;">' +
    ...
    if (data.todayUnSeeFl === 'y') {
        popupHtml +=
            ...
            '                <span class="inp_chk ">' +
            '                    ' + __('오늘 하루 보이지 않음') +
            '                    <input type="checkbox" id="todayUnSee_memberEventPopup_'+ data.eventType +'" onclick="gd_popup_cookie(\'memberEventPopup_'+ data.eventType +'\', this);">' +
            '                </span>' +
            '            </div>';
        <!-- 오늘 하루 보이지 않음 : end -->
    }
